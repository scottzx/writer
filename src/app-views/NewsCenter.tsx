...
import { Search, TrendingUp, Filter, BarChart, PenLine, History, X } from 'lucide-react';
import TimelineAnalysis from './TimelineAnalysis'; // Import Timeline component
import { ViewState } from '../app-types';

// Extend prop types to include navigation handler
interface NewsCenterProps {
  onNavigate?: (view: ViewState) => void;
}

const NewsCard = ({ source, time, title, snippet, score, trend, color, onClick }: any) => (
...
const NewsCenter: React.FC<NewsCenterProps> = ({ onNavigate }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-background border border-border rounded-2xl w-full max-w-6xl h-[90vh] flex flex-col shadow-2xl relative overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden">
              <TimelineAnalysis isModal={true} onProceed={handleProceedToResearch} />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center border-b border-border pb-4 gap-4">
          ...
          <div className="flex items-center gap-4 text-sm text-textSecondary">
            <span className="flex items-center gap-1 cursor-pointer hover:text-primary">排序： 热度 <BarChart size={16} /></span>
          </div>
        </div>
