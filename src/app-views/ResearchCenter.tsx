...
import { Search, Library, AlertTriangle, ShieldCheck, Bookmark, ExternalLink, GripVertical, Plus, CheckSquare, Square, PenTool, ArrowLeft, Clock, MoreVertical, FileText, Sparkles } from 'lucide-react';
...
const FragmentCard = ({ type, source, content, isChecked }: any) => {
...
const ResearchList = ({ onSelectProject }: { onSelectProject: (id: string) => void }) => {
  const projects = [
    {
      id: '1',
      title: '教育中的 AI 变革',
          ...
          {projects.map((project) => (
            ...
            >
              ...
              <div className="pt-4 border-t border-border flex items-center justify-between text-xs text-textSecondary">
                ...
                <span className="flex items-center gap-1 font-medium text-white">{project.progress}% <span className="text-textSecondary font-normal">完成</span></span>
              </div>
...
const ResearchDetail = ({ onBack, onNavigate }: { onBack: () => void, onNavigate?: (view: ViewState) => void }) => {
  return (
    <div className="flex flex-col h-full bg-background relative animate-in slide-in-from-right duration-300">
      ...
      <div className="h-16 border-b border-border bg-background/95 backdrop-blur z-10 flex items-center justify-between px-6 shrink-0">
         ...
         <div className="flex items-center gap-3">
            <span className="text-xs text-textSecondary">已选素材: <span className="text-primary font-bold">5</span></span>
      ...
      <div className="absolute bottom-8 left-0 right-0 flex justify-center z-20 pointer-events-none">
         <div className="bg-surfaceHighlight border border-border shadow-2xl rounded-full p-2 pl-6 pr-2 flex items-center gap-6 pointer-events-auto animate-in slide-in-from-bottom-4">
            ...
            <button 
              onClick={() => onNavigate && onNavigate(ViewState.EDITOR)}
...
// --- Main Container ---
const ResearchCenter: React.FC<ResearchCenterProps> = ({ onNavigate }) => {
  ...
  if (activeProjectId) {
    return <ResearchDetail onBack={() => setActiveProjectId(null)} onNavigate={onNavigate} />;
  }
